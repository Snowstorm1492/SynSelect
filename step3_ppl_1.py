...
SYS_PROMPT = "Suppose you are an helpful AI assistant. Next, we will provide a picture and a question about this picture. In addition, we have also provided a snippet of another AI's thinking process on this issue for your reference. Please provide your analysis and answer this question based on the given image."
...
def get_cot(reply):
    if "</think>" in reply:
        cot = reply.split("</think>")[0]
        return cot[0 : round(0.9 * len(reply))]
    else:
        return reply[0 : round(0.8 * len(reply))]
...
def get_messages(datum, reply):
    ...
    messages = [
        ...
        },
        {
            "role":"user",
            "content": [
                {"type": "text", "text": USER_PROMPT.format(question, cot)},
...
async def main(directory):
    ...
    with open(f"{directory}/evaluation3_v1-{os.path.basename(directory)}.json", "r", encoding="utf-8") as f:
    ...
    with open(f"{directory}/evaluation3_v2-{os.path.basename(directory)}.json", "w", encoding="utf-8") as f:
